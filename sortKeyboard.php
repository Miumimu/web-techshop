...
{
  $result = mysqli_query($conn, "SELECT product_Name,product_Price FROM product_keyboard ORDER BY product_Price");
  mysqli_close($conn);
}
?>
...
if (isset($_POST['reviewSort']))
{
  ...
  if(!mysqli_connect_errno())
  {
    $result = mysqli_query($conn, "SELECT product_Name,product_Review FROM product_keyboard ORDER BY product_Review");
    mysqli_close($conn);
  }
  ?>
